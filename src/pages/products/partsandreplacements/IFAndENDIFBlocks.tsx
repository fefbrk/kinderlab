...
// Import images
import MainImg from '@/assets/product/partsandreplacements/ifandendifblocks/if-end-if.jpg';

const IfAndEndIfBlocks = () => {
  ...
  const productInCart = cartItems.find(item => item.id === '311');
  ...
  const handleAddToCart = () => {
    const product = products.find(p => p.id === '311');
    if (product) {
      addToCart(product);
      setJustAdded(true);
    ...
    toast({
      title: "Added to cart",
      description: "IF And END-IF Blocks set added to your cart.",
    });
  };

  const getPrice = () => {
    const product = products.find(p => p.id === '311');
    return product ? `$${product.price.toFixed(2)}` : '$20.00';
  };
    ...
    <div className="min-h-screen bg-orange-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav className="flex items-center space-x-2 text-sm text-gray-600 mb-6">
          ...
          <span>/</span>
          <span className="text-kibo-purple font-medium">IF And END-IF Blocks</span>
        </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="flex space-x-2">
              {images.map((image, index) => (
                ...
                >
                  <img
                    src={image}
                    alt={`IF And END-IF Blocks view ${index + 1}`}
          ...
          <div className="space-y-6">
            <div>
              <h1 className="text-3xl font-bold text-kibo-purple mb-4">IF And END-IF Blocks</h1>
            ...
            <div className="text-sm text-gray-500 space-y-1">
              <div><span className="font-medium">SKU:</span> BLK-IF-ENDIF</div>
            ...
            {activeTab === 'description' && (
              <div className="prose max-w-none">
                ...
                <p className="text-gray-700 mb-4">
                 The IF and END-IF blocks allow students to extend their use of KIBO with these conditional blocks along with their corresponding parameter cards. These blocks require the Parameters for IF blocks and the light and/or distance sensors.
                  <br></br><br></br> 
       ...
       {isModalOpen && (
         <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50" onClick={() => setIsModalOpen(false)}>
           <div className="relative max-w-4xl max-h-[90vh] p-4">
             ...
             <img
                src={images[selectedImage]}
                alt="IF And END-IF Blocks - Enlarged"
...
export default IfAndEndIfBlocks;