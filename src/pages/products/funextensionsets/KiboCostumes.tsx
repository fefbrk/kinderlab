...
// Import images
import MainImg from '@/assets/product/funextensionsets/kibocostumes/Costume-600x600.jpg';
import ButterflyImg from '@/assets/product/funextensionsets/kibocostumes/Butterfly-Costume.jpeg';
import SuperheroImg from '@/assets/product/funextensionsets/kibocostumes/Superhero-e1658161117861.jpg';
import MixedCostumesImg from '@/assets/product/funextensionsets/kibocostumes/mixed-costumes-with-kibo-e1658068262578.jpg';
...
const KiboCostumes = () => {
  ...
  const productInCart = cartItems.find(item => item.id === '208');
  ...
  const handleAddToCart = () => {
    const product = products.find(p => p.id === '208');
    if (product) {
      addToCart(product);
      setJustAdded(true);
    ...
    toast({
      title: "Added to cart",
      description: "KIBO Costumes added to your cart.",
    });
  };

  const getPrice = () => {
    const product = products.find(p => p.id === '208');
    return product ? `$${product.price.toFixed(2)}` : '$89.00';
  };
    ...
    <div className="min-h-screen bg-orange-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav className="flex items-center space-x-2 text-sm text-gray-600 mb-6">
          ...
          <span>/</span>
          <span className="text-kibo-purple font-medium">KIBO Costumes</span>
        </nav>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="flex space-x-2">
              {images.map((image, index) => (
                ...
                >
                  <img
                    src={image}
                    alt={`KIBO Costumes view ${index + 1}`}
          ...
          <div className="space-y-6">
            <div>
              <h1 className="text-3xl font-bold text-kibo-purple mb-4">KIBO Costumes</h1>
            ...
            <div className="text-sm text-gray-500 space-y-1">
              <div><span className="font-medium">SKU:</span> KCOS208</div>
            ...
            {activeTab === 'description' && (
              <div className="prose max-w-none">
                ...
                <p className="text-gray-700 mb-4">
                 <strong>KIBO has costumes!</strong>  KIBO Costumes expand the options for fun, creative, imagination play with KIBO! Will KIBO wear a fancy green dress to the ball? Is KIBO an orange tiger with stripes, or a super-hero with crazy green hair, a silver mask, and stars on its cape?
                </p>
                
                <p className="text-gray-700 mb-4">
                 These paper cut-outs cover KIBO’s top. Draw on the costume with crayons, markers, pens — anything you’d like! When you’re ready for a change, switch it out for a new costume.
                </p>
                
            
                
                <p className="text-gray-700">
                The KIBO Costume Pack includes <strong>12 assorted double-sided costumes</strong> with different colors and patterns to inspire your children and students! KIBO Costumes work with any KIBO robot kit.
                </p>
              </div>
            )}
                  ...
                  <table className="w-full">
                    <tbody>
                      <tr className="border-b border-gray-200">
                        ...
                        <td className="px-6 py-4 text-gray-700 w-2/3">0.1 lbs</td>
                      </tr>
                      <tr>
                        <td className="px-6 py-4 font-medium text-gray-900 bg-gray-100 w-1/3">Dimensions</td>
                        <td className="px-6 py-4 text-gray-700 w-2/3">8 × 6 × 0.5 in</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
